...
export function uploadExtensions(rules: UploadRules): string[] {
 ...
 */
export function uploadAccept(rules: UploadRules): string {
	return [
...
export function uploadFormatList(rules: UploadRules): string {
 ...
 */
export function uploadFormatForBytes(
	rules: UploadRules,
	bytes: Uint8Array,
): UploadFormat | undefined {
	return rules.formats.find((format) => {
		const headerMatches = format.signatures.every((signature) =>
			signature.bytes.every((byte, i) => bytes[signature.offset + i] === byte),
		);
 ...
 */
export function extensionOf(nameOrUrl: string | undefined): string | undefined {
...
/* ------------------------------------------------------------------ */
/* Résumé download link — the visitor-facing half                      */
/* ------------------------------------------------------------------ */
...
export function resumeFormatLabel(profile: ResumeProfile): string | undefined {
 ...
 */
export function resumeDownload(profile: ResumeProfile): ResumeDownload | null {
